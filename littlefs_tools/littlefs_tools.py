    ...
    try:
        ...
        for item in fsContent:
            ...
            if item.type==2: #type=Folder
                Path(f"{destination}/{path}/{item.name}").mkdir(parents=True,exist_ok=True)
            ...
            for name in files:
                ...
                with open(filePath, 'rb') as file:
                    ...
                    if contentSize > (args.blockSize * (args.blockCount-2)):
                        logger.critical("Contents wont fit into the disk. Plese adjust block size and block count")
                        logger.critical(f"\tTotal Contents size = {contentSize} Bytes")
                        logger.critical(f"\tImage size [{args.blockSize} x {args.blockCount}] = {args.blockSize*args.blockCount} Bytes")
                        exit(-1)
